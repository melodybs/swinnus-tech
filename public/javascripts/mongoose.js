  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      var users = JSON.parse(xhr.responseText), 
          tbody = document.querySelector('#user-list tbody'); //console.log(users);
    ...
    if (xhr.status === 200) {
      ...
      comments.map(function (comment) { 
        ...
        edit.addEventListener('click', function () { // 수정 클릭 시
          ...
          xhr.open('PATCH', '/mongoComments/' + comment._id);
        ...
        remove.addEventListener('click', function () { // 삭제 클릭 시
          ...
          xhr.open('DELETE', '/mongoComments/' + comment._id);
          xhr.send();
        });
